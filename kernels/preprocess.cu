...
__global__ void preprocess_kernel(
    const uint8_t *__restrict__ src, // BGR frame from OpenCV, on GPU
    float *__restrict__ dst,         // output: float32 NCHW
    int src_w, int src_h, int dst_w, int dst_h, float scale, int pad_x,
    int pad_y) {
...
void launch_preprocess(const uint8_t *d_bgr, float *d_out, int src_w, int src_h,
    ...
    preprocess_kernel<<<blocks, threads, 0, stream>>>(
        d_bgr, d_out, src_w, src_h, dst_w, dst_h, scale, pad_x, pad_y);
}
